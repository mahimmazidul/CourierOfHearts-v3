...
const stats = await page.evaluate(() => [...document.querySelectorAll('article.print-letter')].map(a => {
  ...
  return Math.round((last.getBoundingClientRect().bottom - ar.top) / ar.height * 100);
}));
console.log('pages:', pages, '| fill %:', JSON.stringify(stats), '(target: high 80s, never >93)');
const ends = await page.evaluate(() => [...document.querySelectorAll('article.print-letter .rich-letter-content')].map(b => (b.textContent||'').trim().slice(-18)));
console.log('endings intact:', JSON.stringify(ends));
await page.pdf({ path: '/tmp/refill2.pdf', format: 'A4', printBackground: true });
const { readFileSync } = await import('node:fs');
console.log('PDF pages:', (readFileSync('/tmp/refill2.pdf').toString('latin1').match(/\/Type\s*\/Page[^s]/g) || []).length);
await browser.close();
